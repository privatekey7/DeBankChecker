...
from debank_checker.config import APPCHAIN_TIMEOUT, HL_API_URL, HL_RATE_PER_SEC
from debank_checker.parallel import Memo, run_parallel, unwrap
...
def _post(payload: dict, proxy: str | None) -> Any:
    ...
    for attempt in range(attempts):
        _LIMITER.wait(proxy)
        try:
            resp = http.post(HL_API_URL, proxy, json=payload, timeout=APPCHAIN_TIMEOUT)
            ...
            return resp.json()
        except (HyperliquidError, http.ProxyDead):
            raise
