...
from debank_checker.api import http, onchain
from debank_checker.api.http import PerKeyLimiter
from debank_checker.config import APPCHAIN_RATE_PER_SEC, APPCHAIN_TIMEOUT
from debank_checker.diskcache import PersistentDict
from debank_checker.parallel import run_parallel, unwrap
...
def proxy_wallet(address: str, proxy: str | None = None) -> str | None:
    ...
    try:
        res = onchain._call(POLYGON_CHAIN_ID, "eth_call", [{"to": SAFE_FACTORY, "data": data}, "latest"], proxy)
...
def _get_json(path: str, params: dict, proxy: str | None) -> Any:
    ...
    for attempt in range(attempts):
        _LIMITER.wait(proxy)
        try:
            resp = http.get(DATA_API + path, proxy, params=params, timeout=APPCHAIN_TIMEOUT)
            if resp.status_code == 429:
                time.sleep(pause if proxy else min(http.retry_after(resp, 3.0 * (attempt + 1)), 30.0))
                last = PolymarketError("HTTP 429 from Polymarket")
                continue
            if resp.status_code != 200:
                raise PolymarketError(f"Polymarket HTTP {resp.status_code} ({path})")
            return resp.json()
        except (PolymarketError, http.ProxyDead):
            raise
